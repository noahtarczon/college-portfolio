    ...
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        if (data.event == "onTime") {
          ...
          let rev = Number(data.details["revenue"]);
          rev = Math.round(rev*100) / 100;
          let triprev = Number(data.details["tripRevenue"]);
          triprev = Math.round(triprev*100) / 100;
          let notif_string;
          notif_string = name + " has made $" + String(triprev) + " and now has $" + String(rev) + "\r\n" + "Thank you for your order!" + "\r\n\r\n";
          notifbar = document.getElementById("notification-bar");
          notifbar.textContent += notif_string;
        }
        if (data.event == "notOnTime") {
          console.log(data.details);
          let name = data.details["name"];
          let rev = Number(data.details["revenue"]);
          rev = Math.round(rev*100) / 100;
          let notif_string;
          notif_string = name + " has made $0 and now has $" + String(rev) + "\r\n" + "Drone didn't make it in time :(" + "\r\n\r\n";
          notifbar = document.getElementById("notification-bar");
          notifbar.textContent += notif_string;
        }
        if (data.event == "gold") {
          ...
          let name = data.details["name"];
          let notif_string;
          notif_string = name + " has reached GOLD STATUS! WOW!" + "\r\n\r\n";
          notifbar = document.getElementById("notification-bar");
          notifbar.textContent += notif_string;
        }
        if (data.event == "Scheduled"){
          let notif_string;
          let package_name = data.details["package_name"];
          let name = data.details["name"];
          let speed = data.details["speed"];
          let delivery = "";
          let limit = 0; 
          if (speed == 30.0){
            delivery = "Standard";
          ...
          if (speed == 30.0){
            notif_string = name + " has scheduled "+ package_name + " for pick-up at a " + delivery + " delivery speed." + "\r\n\r\n";
          }else{
            notif_string = name + " has scheduled "+ package_name + " for pick-up at a " + delivery + " delivery speed." + "\r\n" + "The drone will deliver in " + limit + " seconds or your package is on us!" + "\r\n\r\n";
          }
          notifbar = document.getElementById("notification-bar");
          notifbar.textContent += notif_string;
        }
