...
int main(int argc, char **argv) {
    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        if (strcmp(first_token, "pwd") == 0) {
            ...
            const char* directory = strvec_get(&tokens, 1);
            char *environment = getenv("HOME"); 
            if (environment == NULL) {
                perror("Failed to get HOME directory");
            }
            if (directory == NULL) {
                if (chdir(environment) == -1) {
                    perror("Failed to change to HOME");
                }
            }
            else {
        ...
        // Task 6: Wait for all background jobs
            ...
            pid_t cpid = fork();
            int status;
            //MOVE INDEX HERE AND CHECK IF & IS IN TOKENS INSIDE CHILD
            int index = strvec_find(&tokens, "&");
            const char *name = strvec_get(&tokens, 0);
            if (cpid == 0) {
                if (index != -1) {
                    strvec_take(&tokens, index);
                    
                }
                int result = run_command(&tokens);
                if (result == -1) {
                    return 1;
                }
            }
                ...
                if (index != -1) {
                    job_list_add(&jobs, cpid, name, JOB_BACKGROUND);
                }
                if (index == -1) {
                    if(tcsetpgrp(STDIN_FILENO, cpid)==-1){      //task 4------
                        perror("tcsetpgrp: cpid");
                        return 1;
                    }
                    pid_t waiting = waitpid(cpid, &status, WUNTRACED);    //wait until the process is done
                    if(waiting < 0) {
                        printf("call to \"waitpid()\" failed.\n");
                    }
                    if(tcsetpgrp(STDIN_FILENO, getpid())==-1){ //task 4------
                        perror("tcsetpgrp: getpid");
                        return 1;
                    }
                    if (WIFSTOPPED(status)) {           //task 5------
                        if (job_list_add(&jobs, cpid, name, status) == -1) {
